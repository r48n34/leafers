...
# model_dir = "C:\\fypData\\flower245.h5"
# data_dir = "C:\\fypData\\groupByNew"
...
def predictTopkAll(topK, file_paths_arr, labels):
    ...
    for ind, imgPath in enumerate(file_paths_arr):
        ...
        # sum predictions
        prediction = prediction_One[0] + prediction_Two[0] + prediction_Three[0]

        # print(prediction)

        predictionArgsort = np.argsort(prediction)
        ...
        for i in range(topK):
            logging.info("Predict top " +  str(i + 1) + " " +  str( prediction[ predictionArgsortReverse[i] ] ) +  " : " +  str( labels[ predictionArgsortReverse[i] ]))
...
logName = "embCode9_10_11Report.log"
