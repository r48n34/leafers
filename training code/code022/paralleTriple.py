...
def create_model():
    ...
    model021 = tf.keras.models.load_model("E:\\test vsxc code\\fyp\\fypData\\model\\021\\offv2b1RRModel.h5")
    model021.trainable = False
    model021._name = "model021"
    ...
    img240 = tf.keras.layers.Resizing(240, 240)(input_shape)
    m3 = model021(img240)

    merged = tf.keras.layers.concatenate([m1,m2,m3], axis=1)
    merged = tf.keras.layers.Flatten()(merged)

    do = tf.keras.layers.Dropout(0.2)(merged)
